"""Room model that holds participants and enforces capacity rules."""
...
class RoomFullError(RoomError):
    """Raised when adding a participant exceeds room capacity."""


class ParticipantNotInRoomError(RoomError):
    """Raised when an operation expects a participant in a room."""


class DuplicateParticipantError(RoomError):
    """Raised when the same participant is added twice to one room."""


class RoomNotFoundError(RoomError):
    """Raised when a room lookup fails for an expected existing room."""
