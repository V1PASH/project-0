"""
transport/webrtc/manager.py
RoomManager — the single source of truth for all active rooms and participants.

All public methods are coroutine-safe (they don't use asyncio locks themselves
but are designed to be called from a single-threaded asyncio event loop, which
is the normal deployment model for this framework).
"""
...
from transport.webrtc.participants.base import ParticipantState
from transport.webrtc.participants.participant import (
    LocalParticipant,
    Participant,
    RemoteParticipant,
)
from transport.webrtc.room.room import DuplicateParticipantError, Room, RoomFullError
...
class RoomManager:
    """
    Creates, tracks, and destroys rooms.
    Also maps WebSocket connections → Participants for fast lookup.
    """
    ...
    def __init__(self, event_history_size: int = 200) -> None:
        ...
        # websocket object id → participant_id  (for disconnect handling)
    ...
    def _emit_event(self, event: RoomEvent) -> None:
        ...
        # Built-in room event broadcast to connected peers.
    ...
    def register_participant(self, websocket: object, name: str = "Anonymous") -> Participant:
        """Create a Participant for a newly connected WebSocket."""
    ...
    def register_remote_participant(
        ...
        """Create a remote participant object (no direct websocket attached)."""
    ...
    def unregister_participant(self, websocket: object) -> Optional[Participant]:
        """
        Remove the participant associated with this WebSocket.
        Also removes them from their room if they're in one.
        Returns the removed Participant or None.
        """
    ...
    def get_or_create_room(
        ...
        """Return an existing room or create a new one."""
    ...
    def join_room(
        self, participant: Participant, room_id: str, max_participants: int = 10
    ) -> Room:
        """
        Add a participant to a room (creating it if needed).
        Raises RoomFullError if the room is at capacity.
        """
        ...
        log.info("Participant %s joined room %s (%d/%d)",
                 participant.participant_id, room_id,
                 room.participant_count, room.max_participants)
        return room

    def update_participant_speaking(self, participant: Participant, speaking: bool) -> Optional[RoomEvent]:
        """Emit speaking activity for an in-room participant when state changes."""
    ...
    def emit_participant_transcript(
        ...
        *,
        ...
        """Emit a transcript update for an in-room participant."""
        if not participant.room_id:
            return None
    ...
    def leave_room(self, participant: Participant) -> Optional[Room]:
        """Remove a participant from their current room. Destroys the room if empty."""
        if not participant.room_id:
            return None
        room_id = participant.room_id
        room = self._rooms.get(room_id)
